CREATE DATABASE if not exists ADFFLOW1;
CREATE EXTERNAL TABLE if not exists ADFFLOW1.FAAMaster
(
    N_Number string,
...
LOCATION 'wasb://input@ADFFLOW1staging.blob.core.windows.net/master' TBLPROPERTIES("skip.header.line.count"="1");

CREATE EXTERNAL TABLE if not exists ADFFLOW1.aircraft
(
    MFR_NAME string,
	MODEL_NAME string,
	TYPE_AIRCRAFT string,
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' lines terminated by '\n'
LOCATION 'wasb://input@ADFFLOW1staging.blob.core.windows.net/FAAaircraft' TBLPROPERTIES("skip.header.line.count"="1");

CREATE TABLE if not exists ADFFLOW1.Dummy
(
	Some_key tinyint
);
...
insert overwrite directory 'wasb://output@ADFFLOW1staging.blob.core.windows.net/FAAMerge' 
